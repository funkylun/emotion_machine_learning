...
def get_filelist(detector,model,dir):
    ...
    for home, dirs, files in os.walk(dir):
        # for dir in dirs:
        #     print(dir)
        for root, dirs2, files2 in os.walk("./output/2022/mild/"):
...
if __name__=="__main__":
    ...
    # testPath = "G:/第一次（27正常+39痴呆）/正常/"
    # get_filelist(detector,net,testPath)
    # testPath = "G:/第二次（30正常+15痴呆）/正常/"
    # get_filelist(detector,net,testPath)
    # testPath = "G:/第三次（70正常+70痴呆）/正常/"
    # get_filelist(detector,net,testPath)
    testPath = "G:/计算机老师/第一次（12正常+15轻度认知障碍+39痴呆）2021.12.22/轻度认知障碍/"
    get_filelist(detector, net, testPath)
    testPath = "G:/计算机老师/第二次（19正常+5轻度认知障碍+21痴呆）2021.12.22/轻度认知障碍/"
    get_filelist(detector, net, testPath)
    testPath = "G:/计算机老师/第三次（26正常+42轻度认知障碍+72痴呆）2021.12.22/轻度认知障碍/"
    get_filelist(detector, net, testPath)
    testPath = "G:/计算机老师/第四次（39正常+36轻度认知障碍+6痴呆）2021.12.22/轻度认知障碍/"
    get_filelist(detector, net, testPath)
